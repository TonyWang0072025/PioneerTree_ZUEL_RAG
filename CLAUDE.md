...
Phase 9 视觉重构 + resourcesPath 修复完成。准备进入打包测试阶段。

## 下一步

1. 在 Electron 中启动应用 (`npm run electron:dev`)，实测拖拽/折叠/Tabs 切换效果
...
最近一批改动（Phase 9 — 视觉重构 + UI 清理 + 侧边栏交互）：
- `src/assets/main.css` — 全量重写设计令牌 + 70+ 全局类 + tabs/resize-handle/panel-toggle 样式
- `src/App.vue` — 侧边栏布局 + 删除账户 UI + API 连接状态检测
- `src/views/LawSubject.vue` — 模板重构（设计规范适配）+ AI tutor 接入右侧栏 Tabs
- `src/components/SubjectLayout.vue` — 拖拽 resize + 折叠 + showAiTab/Tabs 机制
- `src/components/NotebookEditor.vue` — notes-header/notes-btn 样式对齐
- `CLAUDE.md` — 更新项目状态
